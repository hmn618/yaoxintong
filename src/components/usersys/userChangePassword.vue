		    ...
		    <div class="menu" style="background-color: #67B168;">
		      ...
		      <menus>
		        <menu-item icon="image">
		          <span>个人信息</span>
		          <menus slot="sub">
		            <menu-item :to="{ path: '/usercenter'}" icon="car" >信息展示</menu-item>  <!--, query: { userId: 321 }-->
		          <!--  <menu-item :to="{ path: '/userchangeinfor'}" icon="slack">修改基本信息</menu-item>-->
		            <menu-item :to="{ path: '/userchangepassword'}" icon="slack" :is-active="true">修改密码</menu-item>
		          </menus>
		        </menu-item>
	...
	export default{
		...
		data(){
  		...
  		methods:{
  			...
  			submitpsw:function(){
  				this.$http.post(this.HOST+'/user/changepassword',{password:this.prepsw,new_password:this.newpsw,token:this.token},{emulateJSON:true})
  				.then((response)=>{
