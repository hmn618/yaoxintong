  	...
  	通过 label 设置菜单组的标签
		...
		<div class="columns" style="background-color: #FFFFFF;">
			      ...
			      <menus>
			        <menu-item icon="image">
			          ...
			          <menus slot="sub">
			            ...
			            <menu-item :to="{ path: '/changesellerinfor'}" icon="slack" :is-active="true">修改基本信息</menu-item>
			...
			<div class="column is-6">
				<tabs type="boxed">
			        <tab-item label="商家信息修改">
						<div class="kuai">
							<label for="orderid " class="la ">账户名：</label>
							<input type="text " v-model='username' name="companyname1 " id="companyname1 " class="form-control " style="width: 240px; ">
						</div>
						<div class="kuai">
							<label for="medicine_name " class="la ">公司名称：</label>
							<input type="text " v-model='companyname' name="user " id="user " class="form-control " style="width: 240px; ">
						</div>
						<div class="kuai">
						...
						<div class="kuai">
							...
							<input type="text " v-model='contactman' name="user " id="user " class="form-control " style="width: 240px; ">
						</div>
						<div class="kuai">
  	...
  	methods:{
			modify:function(){
				if(this.token==null){
					...
					this.$http.post(this.HOST+'/business/modify',{username:this.username,businessName:this.companyname,email:this.email,
				   phone:this.phone,contact:this.contactman,token:this.token},{emulateJSON:true})
					.then((response)=>{
						console.log(response);
	  				var company=response.body;
	  				if(company.result==true){
